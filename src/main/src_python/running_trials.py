	...
	def run_parallel_trials(self, games, trials, contexts, ais, n_objects):
		...
		with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
			...
			for i in range(MAX_WORKERS):
				fs.append(executor.submit(run_trial, games[i], trials[i], contexts[i], ais[i]))
...
def run_trial(game, trial, context, ais):
	...
	print("* AI1 winrate:", ai1_win/(total+1))
	print("* AI2 winrate:", ai2_win/(total+1))
